...
export async function buildFeasibilityWorkbook(
  ...
  const assumptions: [string, number, string, string][] = [
    ...
    ["대출비율 (LTC)", input.loanRatio, PCT, "직접사업비 대비"],
    ["조달금리 (연)", input.interestRate, "0.00%", ""],
    ["평균 인출률", input.drawdownRatio, PCT, "이자 계산용"],
    ["PF 취급수수료율", input.pfFeeRatio, "0.00%", "대출원금 기준 일시"],
    ["사업기간 (개월)", input.periodMonths, NUM, ""],
  ];
  const A0 = 5; // 첫 가정 행
  ...
  const R = {
    ...
    sp: `B${A0 + 8}`, sls: `B${A0 + 9}`, rsv: `B${A0 + 10}`, ltc: `B${A0 + 11}`,
    ir: `B${A0 + 12}`, dd: `B${A0 + 13}`, fee: `B${A0 + 14}`, pm: `B${A0 + 15}`,
  };
  ...
  const proforma: [string, string, string][] = [
    ["연면적 (평)", `${R.area}*${R.far}`, NUM],
    ["분양면적 (평)", `B${P0}*${R.sal}`, NUM],
    ["분양수입", `B${P0 + 1}*${R.sp}*${R.sell}`, NUM],
    ["토지비", `${R.area}*${R.lp}`, NUM],
    ["취득부대비", `B${P0 + 3}*${R.acq}`, NUM],
    ["공사비", `B${P0}*${R.cc}`, NUM],
    ["설계·감리비", `B${P0 + 5}*${R.dsg}`, NUM],
    ["예비비", `(B${P0 + 3}+B${P0 + 5})*${R.rsv}`, NUM],
    ["직접사업비 계", `SUM(B${P0 + 3}:B${P0 + 7})`, NUM],
    ["판매비", `B${P0 + 2}*${R.sls}`, NUM],
    ["대출원금", `B${P0 + 8}*${R.ltc}`, NUM],
    ["이자비용", `B${P0 + 10}*${R.dd}*${R.ir}*${R.pm}/12`, NUM],
    ["PF 수수료", `B${P0 + 10}*${R.fee}`, NUM],
    ["총사업비", `B${P0 + 8}+B${P0 + 9}+B${P0 + 11}+B${P0 + 12}`, NUM],
    ["개발이익 (세전)", `B${P0 + 2}-B${P0 + 13}`, NUM],
    ["사업이익률 (÷총사업비)", `B${P0 + 14}/B${P0 + 13}`, PCT],
    ["매출액이익률", `B${P0 + 14}/B${P0 + 2}`, PCT],
    ["자기자본 (Equity)", `B${P0 + 13}-B${P0 + 10}`, NUM],
    ["자기자본수익률", `B${P0 + 14}/B${P0 + 17}`, PCT],
    ["연환산 자기자본수익률", `B${P0 + 18}/(${R.pm}/12)`, PCT],
  ];
  proforma.forEach(([label, formula, fmt], idx) => {
    ...
    v.value = { formula };
    v.numFmt = fmt;
    if (["직접사업비 계", "총사업비", "개발이익 (세전)"].includes(label)) {
  ...
  SENSITIVITY_DELTAS.forEach((dc, i) => {
    ...
    SENSITIVITY_DELTAS.forEach((_, j) => {
      ...
      const direct = `(${land})*(1+${R.acq})+(${cons})*(1+${R.dsg})+((${land})+(${cons}))*${R.rsv}`;
      const fin = `(${direct})*${R.ltc}*(${R.dd}*${R.ir}*${R.pm}/12+${R.fee})`;
      const total = `(${direct})+(${rev})*${R.sls}+(${fin})`;
